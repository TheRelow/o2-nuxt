<template>
  <div>
    ...
    <input type="text" placeholder="Номер телефона" @input="updatePhoneNumber" class="offer__input" :class="{'offer__input_invalid': $v.value.number.$dirty && (!$v.value.number.required || !$v.value.number.minLength)}" ref="phone">
  </div>
</template>
