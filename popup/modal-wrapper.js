    ...
    Bus.$on('new', options => {
      ...
      if(options.component.template) {
        ...
        try {
          ...
          console.log('какая-то непонятная ошибка с отсутвием компонента в компоненте. Можно легко пофиксить, но это будет какое-то странное решение')
          console.log('при том, если обработать эту ошибку через try, catch - то проблемм нет. Пока решение не нашел')
        }
      }
