    ...
    Bus.$on("new", (options) => {
      ...
      if (options.component.template) {
        ...
        try {
          ...
          console.log(
            "какая-то непонятная ошибка с отсутвием компонента в компоненте. Можно легко пофиксить, но это будет какое-то странное решение"
          );
          console.log(
            "при том, если обработать эту ошибку через try, catch - то проблемм нет. Пока решение не нашел"
          );
        }
      }
      ...
      if (
        rendered &&
        rendered.componentOptions &&
        rendered.componentOptions.Ctor.extendOptions.componentName ===
          "vu-modal"
      ) {
    ...
    let modals = this.modals.map((modal, index) => {
      ...
      if (modal.isVmodal) {
        modalComponent = h(modal.options.component, {
          props: Object.assign(
            {},
            {
              vModal: Object.assign(modal.options, {
                disabled: index != this.$last,
              }),
            },
            modal.options.props
          ),
        ...
        modalComponent = h(
          ModalCmp,
          {
            props: Object.assign(modal.options, {
              disabled: index != this.$last,
            }),
          },
          [
      ...
      return h(
        "div",
        {
          class: [
            "vu-modal__mask",
            { "vu-modal__mask--disabled": index !== this.$last },
          ],
          on: {
            mousedown: (e) => {
              console.log("e", e);
              if (
                e?.srcElement &&
                e.srcElement[0]?.className === "vu-modal__mask"
              ) {
